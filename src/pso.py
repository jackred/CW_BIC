...
class Particle:
    def __init__(self, dimension, min_bound, max_bound, fitness_function,
                 comparator=maximise, version=2007,
    ...
    * new velocity is :
    w: inertia
    vi(t+1) = w * v(i) + x'i(t) + xi(t)
    * which means the next position will be:
    xi(t+1) = w * v(i) + x'i(t)
    """
    ...
    move the particle following the method of PSO 2007
    confine the particle at the boundary of the search space
    and divide by 2 the velocit if it hit the wall
    """
    def move_2011(self):
        for i in range(self.dimension):
            new_pos = self.position[i] + self.velocity[i]
